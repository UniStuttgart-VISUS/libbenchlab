 ...
 */
excel_worker::excel_worker(_Inout_ visus::benchlab::unique_handle&& input,
 ...
 * excel_worker::start
 */
void excel_worker::start(_In_ const std::chrono::milliseconds interval) {
    assert(this->_input != nullptr);

    // Start streaming data from the given Powenetics device.
    {
        auto hr = benchlab_start_streaming(this->_input.get(), interval.count(),
...
void excel_worker::stop(void) {
    ...
    ::benchlab_stop_streaming(this->_input.get());
 ...
 */
void excel_worker::callback(_In_ benchlab_handle source,
...
void excel_worker::worker(void) {
    ...
    auto running = true;
    std::vector<benchlab_sample> samples;
